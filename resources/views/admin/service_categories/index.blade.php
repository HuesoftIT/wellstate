...
@section('main-content')
    <div class="box">
        <div class="content-header border-bottom pb-5">
            ...
            @can('ServiceCategoryController@store')
                <a href="{{ url('/admin/service-categories/create') }}" class="btn btn-default float-right">
        ...
        <div class="box-header">
            <div class="box-tools">
                {!! Form::open(['method' => 'GET', 'url' => '/admin/service-categories', 'class' => 'pull-left']) !!}
        ...
        @php($index = ($serviceCategories->currentPage() - 1) * $serviceCategories->perPage())
            ...
            <table class="table table-bordered table-hover">
                <tbody>
                    <tr>
                        ...
                        <th>@sortablelink('order', 'Thứ tự')</th>
                    ...
                    @foreach ($serviceCategories as $item)
                        <tr>
                            ...
                            @can('ServiceCategoryController@show')
                                <td>
                                    <a href="{{ url('/admin/service-categories/' . $item->id) }}" style="color:black">
                                        {{ $item->name }}
                                    </a>
                                </td>
                            @endcan
                            ...
                            <td class="dropdown">
                                ...
                                <div class="dropdown-menu p-0">
                                    @can('ServiceCategoryController@show')
                                        <a href="{{ url('/admin/service-categories/' . $item->id) }}"
                                    ...
                                    @can('ServiceCategoryController@update')
                                        <a href="{{ url('/admin/service-categories/' . $item->id . '/edit') }}"
                                    ...
                                    @can('ServiceCategoryController@destroy')
                                        {!! Form::open([
                                            'method' => 'DELETE',
                                            'url' => ['/admin/service-categories', $item->id],
                ...
                @if ($serviceCategories->count() == 0)
                    <tr>
                        <td class="text-center" colspan="9">{{ trans('services.no_item') }}</td>
                    </tr>
                @endif
            </table>
        </div>
        <div class="box-footer clearfix">
            <div id="btn-act">
                @can('ServiceCategoryController@destroy')
                    <a href="#" id="deleteServiceCategories" data-action="deleteServiceCategories"
                        class="btn-act btn btn-danger btn-sm" title="{{ __('message.delete') }}">
                        <i class="fas fa-trash-alt" aria-hidden="true"></i>
                    </a>
                @endcan
                &nbsp;
                @can('ServiceCategoryController@active')
                    <a href="#" id="activeServiceCategories" data-action="activeServiceCategories"
            ...
            <div class="page-footer pull-right">
                {!! $serviceCategories->appends(\Request::except('page'))->render() !!}
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function ajaxCategory(action) {
            ...
            let actTxt = '',
                ...
                classAlert = '';
            switch (action) {
                case 'activeServiceCategories':
                    actTxt = 'duyệt';
                    classAlert = 'alert-success';
                    break;
                case 'deleteServiceCategories':
                    actTxt = 'xóa';
