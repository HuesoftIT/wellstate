...
@section('main-content')
    <div class="box">
        <div class="content-header border-bottom pb-5">
            ...
            @can('ImageCategoryController@store')
                <a href="{{ route('image-categories.create') }}" class="btn btn-default float-right">
        ...
        <div class="box-header">
            <div class="box-tools">
                {!! Form::open([
                    'method' => 'GET',
                    'url' => route('image-categories.index'),
                    'class' => 'pull-left',
                ]) !!}
                ...
                <div class="input-group" style="display:flex; gap:8px;">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" class="form-control input-sm"
                        placeholder="{{ __('images.category_name') }}" style="width:250px;">
                ...
                <thead>
                    <tr>
                        ...
                        <th class="text-center" width="12%">
                            {{ __('images.total_images') }}
                        </th>

                        {{-- STATUS --}}
                        <th class="text-center" width="8%">{{ __('message.active') }}</th>

                        {{-- CREATED --}}
                        <th width="12%">{{ __('news.created_at') }}</th>

                        {{-- UPDATED --}}
                        <th width="12%">{{ __('news.updated_at') }}</th>
                ...
                <tbody>
                    @forelse ($categories as $item)
                        <tr>
                            ...
                            {{-- NAME --}}
                            <td>
                                @can('ImageCategoryController@show')
                                    <a href="{{ route('image-categories.show', $item->id) }}" style="color:blue">
                                        {{ $item->name }}
                                    </a>
                                @else
                                    {{ $item->name }}
                            ...
                            <td class="dropdown">
                                ...
                                <div class="dropdown-menu p-0">
                                    @can('ImageCategoryController@show')
                                        <a href="{{ route('image-categories.show', $item->id) }}"
                                    ...
                                    @can('ImageCategoryController@update')
                                        <a href="{{ route('image-categories.edit', $item->id) }}"
                                    ...
                                    @can('ImageCategoryController@destroy')
                                        {!! Form::open([
                                            'method' => 'DELETE',
                                            'route' => ['image-categories.destroy', $item->id],
        ...
        <div class="box-footer clearfix">
            <div id="btn-act">
                @can('ImageCategoryController@destroy')
                    <a href="#" id="deleteImageCategories" data-action="deleteImageCategories"
                        class="btn-act btn btn-danger btn-sm" title="{{ __('message.delete') }}">
                        <i class="fas fa-trash-alt"></i>
                    </a>
                @endcan
                &nbsp;
                @can('ImageCategoryController@active')
                    <a href="#" id="activeImageCategories" data-action="activeImageCategories"
    ...
    <script type="text/javascript">
        ...
        function ajaxCategory(action) {
            ...
            let actTxt = '',
                ...
                classAlert = '';
            switch (action) {
                case 'activeImageCategories':
                    actTxt = 'duyệt';
                    classAlert = 'alert-success';
                    break;
                case 'deleteImageCategories':
                    actTxt = 'xóa';
            ...
            if (chkId.length != 0) {
                ...
                swal("Lỗi!", 'Vui lòng chọn danh mục hình ảnh để  ' + actTxt + '!', "error")
            }
        }
    </script>
@endsection
